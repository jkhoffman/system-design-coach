...
function start() {
  ...
  assert.equal(commands.completeSessionStart(session.id, commands.reserveSessionStart(session.id), "live_test"), true);
  return session.id;
}

test("newer checkpoints win; finish freezes content across retries and late progress", () => {
  const id = start();
  assert.equal(commands.saveSessionProgress(id, { revision: 2, ...content("new") }), true);
  assert.equal(commands.saveSessionProgress(id, { revision: 1, ...content("old") }), false);
  assert.equal(db.getSession(id).transcript[0].text, "new");
  assert.equal(commands.finishSession(id, { endedAt: Date.now(), ...content("final") }), "saved");
  assert.equal(commands.saveSessionProgress(id, { revision: 3, ...content("late") }), false);
  assert.equal(commands.finishSession(id, { endedAt: Date.now(), ...content("retry") }), "already_finished");
...
test("a progress body arriving after finish cannot overwrite the saved transcript", async () => {
  ...
  const finish = await PATCH(new Request("http://local", { method: "PATCH", body: JSON.stringify({ kind: "finish", endedAt: Date.now(), ...content("final") }) }), ctx);
  controller.enqueue(new TextEncoder().encode(JSON.stringify({ kind: "progress", revision: 1, ...content("old") })));
  controller.close();
...
test("connection reservations reject duplicates and fence expired, released, or finished attempts", (t) => {
  ...
  commands.finishSession(id, { endedAt: clock.now(), ...content("finished") });
...
test("concurrent live-session requests perform one external create", async (t) => {
  ...
  const request = () => new Request("http://local", { method: "POST", body: JSON.stringify({ sessionId: id, sdp: "offer" }) });
